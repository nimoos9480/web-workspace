...
- javax.servlet.ServletRequest를 상속받음
...
1. 첫번째 요청일 경우, 서블릿 객체를 생성하고 init() 메서드를 호출한다. (단, 한번만 수행!)
...
- EL(Expression Language)은 JSP 2.0버전에서 추가된 표현 언어
...
- paramValues : 전달된 파라미터들을 배열로 받아올 때 사용(값이 여러개)
...
```jsp
<c:set vare="num" value="100" scope="request" />
```
